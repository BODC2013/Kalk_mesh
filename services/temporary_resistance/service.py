...
# выбор справочных значений для сравнения временного сопротивления и угла загиба в градусах
    ...
    def sorting(self):
        ...
        return schema.models_class.ResponseCalculateModel(
            ...
            thickness_initial_plate =self.thickness_initial_plate,
            sample_thickness =self.sample_thickness,
            width_thickness = self.width_thickness,
            temporary_resistance =self.temporary_resistance,
...
class Test:
    ...
    def __init__ (self, cmd: schema.models_class.TestModel):
        self.name = cmd.name
        self.steel_grade = cmd.steel_grade
        self.brand = cmd.brand
        self.thickness_initial_plate = cmd.thickness_initial_plate
        ...
        self.breaking_force = List[float]
        self.sample_thickness = cmd.sample_thickness
        self.width_thickness = cmd.width_thickness
...
root = Test(cmd=schema.models_class.TestModel(name="Lilit", steel_grade="09Г2С", brand="vi"))
# low = Calculate(
